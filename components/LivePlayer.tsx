...
function PlayerInterno({ nome }: { nome: string }) {
  ...
  // Receber sinais do director e mensagens de chat
  useEffect(() => {
    const handler = (payload: Uint8Array, participant: any) => {
      try {
        const msg = JSON.parse(new TextDecoder().decode(payload))
        if (msg.tipo === 'director') {
          setCameraAtiva(msg.cameraId)
        } else if (msg.tipo === 'chat') {
          setMensagens(prev => [...prev, {
            autor: participant?.identity || 'Anónimo',
            texto: msg.texto,
          }])
        }
      } catch {}
  ...
  // Auto-selecionar primeira câmera se director não enviou sinal
  useEffect(() => {
    if (!cameraAtiva && videoTracks.length > 0) {
      setCameraAtiva(videoTracks[0].participant.identity)
    }
  }, [videoTracks.length])
  ...
  // Track de vídeo da câmera seleccionada pelo director
  const videoAtivo = cameraAtiva
    ? videoTracks.find(t => t.participant.identity === cameraAtiva)
    : videoTracks[0]

  const audioAtivo = cameraAtiva
    ? audioTracks.find(t => t.participant.identity === cameraAtiva)
    : audioTracks[0]

  const temVideo = !!videoAtivo
  ...
  function enviarMensagem(e: React.FormEvent) {
    ...
    setMensagens(prev => [...prev, { autor: nome, texto: texto.trim() }])
    setTexto('')
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
        ...
        {temVideo ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-zinc-600">
              ...
              <p className="text-sm">À espera da transmissão...</p>
            </div>
          </div>
        )}

        {audioAtivo && <AudioView trackRef={audioAtivo} />}
      ...
      {/* Chat */}
      <div
        className={`lg:w-80 flex flex-col bg-zinc-900 rounded-xl overflow-hidden border border-zinc-800 ${chatAberto ? 'flex' : 'hidden lg:flex'}`}
        style={{ minHeight: '300px' }}
      >
        <div className="px-4 py-3 border-b border-zinc-800 font-medium text-sm flex justify-between items-center">
          <span>Chat ao vivo</span>
          <button onClick={() => setChatAberto(false)} className="lg:hidden text-zinc-500 text-lg">✕</button>
        </div>
        <div ref={chatRef} className="flex-1 overflow-y-auto p-3 space-y-2" style={{ maxHeight: '400px' }}>
          {mensagens.length === 0 && (
            <p className="text-zinc-600 text-sm text-center mt-6">Nenhuma mensagem ainda.</p>
          )}
          {mensagens.map((m, i) => (
